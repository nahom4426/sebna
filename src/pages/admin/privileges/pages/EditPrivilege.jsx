...
import { getPrivilegeById, updatePrivilege } from '@/pages/admin/api/AdminApi';
...
const EditPrivilege = () => {
  ...
  useEffect(() => {
    if (privilegeUuid && !Object.keys(privilege).length) {
      req.send(
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    updateReq.send(
      () => updatePrivilege(privilegeUuid, formData),
      (res) => {
        if (res.success) {
          setPrivilege((prev) => ({ ...prev, ...formData }));
          toasted(true, 'Successfully Updated', 'Privilege has been updated');
          setTimeout(() => {
            navigate(-1);
          }, 1500);
        } else {
          toasted(false, 'Error', res.error || 'Failed to update privilege');
        }
      }
    );
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 sm:px-8 py-8">
            <h1 className="text-3xl sm:text-4xl font-bold text-white">Update Privilege</h1>
            <p className="text-indigo-100 mt-2">Manage privilege details and settings</p>
          </div>
            ...
            {req.pending ? (
              <div className="flex flex-col items-center justify-center py-16">
                <div className="relative w-12 h-12 mb-4">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-8 h-8 bg-indigo-600 rounded-full animate-spin"></div>
                  </div>
                </div>
                <p className="text-gray-600 font-medium">Loading privilege...</p>
              </div>
            ) : (
              <form onSubmit={handleUpdate}>
                ...
                <div className="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-200">
                  ...
                  >
                    {updateReq.pending ? 'Updating...' : 'Update Privilege'}
                  </Button>
                </div>
              </form>
            )}
